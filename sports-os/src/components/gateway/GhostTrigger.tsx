    ...
    <div className="flex flex-col items-end group relative select-none z-50">
      ...
      <div className="absolute top-11 right-0 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300 delay-150 group-hover:delay-0 transform translate-y-1 group-hover:translate-y-0 bg-[#1A1D23]/85 backdrop-blur-[12px] border border-[rgba(156,163,175,0.15)] rounded shadow-2xl w-48 text-right z-50 after:content-[''] after:absolute after:w-full after:h-6 after:-top-5 after:left-0 after:block">
        <button
          ...
          className="w-full text-right text-[#9CA3AF] hover:text-[#00E676] font-mono text-[13px] px-6 py-5 transition-colors cursor-pointer block uppercase"
        >
          01 // ENTER THE CORE
        </button>
        <Link
          href="/sla"
          className="w-full text-right text-[#9CA3AF] hover:text-[#00E676] font-mono text-[13px] px-6 py-5 transition-colors block border-t border-[rgba(156,163,175,0.15)] uppercase"
        >
          02 // PROTOCOL (SLA)
        </Link>
      </div>
