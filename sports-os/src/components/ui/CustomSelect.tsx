...
export default function CustomSelect({ options, selected, onChange, label, disabled = false }: CustomSelectProps) {
  ...
  return (
    <div ref={containerRef} className="relative font-mono text-[10px] w-full mb-6 select-none">
        ...
        className={`w-full flex justify-between items-center bg-transparent border-b py-2.5 px-1 focus:outline-none transition-all uppercase ${
          disabled 
            ...
            : isOpen
              ...
              : 'border-[#9CA3AF]/20 text-[#F5F5F7] hover:border-[#00E676]'
        }`}
      >
      ...
      {isOpen && !disabled && (
        <ul className="absolute z-50 w-full mt-1 bg-[#121418]/95 backdrop-blur-md border border-[#00E676]/30 text-[#F5F5F7] shadow-[0_0_20px_rgba(0,230,118,0.06)] py-1 text-sm font-sans uppercase no-scrollbar">
          {options.map((opt) => (
            <li
              key={opt}
              onClick={() => { onChange(opt); setIsOpen(false); }}
              className="px-3 py-2 cursor-pointer hover:bg-[#00E676]/10 hover:text-[#00E676] transition-all flex items-center gap-1 group normal-case"
            >
