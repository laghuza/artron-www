...
export default function NodeDetailStage({ data, onClose }: NodeDetailStageProps) {
  return (
    <div className="w-full max-w-xl bg-[#1A1D23]/90 border border-[#9CA3AF]/18 backdrop-blur-[12px] p-6 rounded-md shadow-[0_0_8px_rgba(0,230,118,0.12)] animate-fadeIn font-sans text-left relative z-40 space-y-5">
      {/* Top Header Bar */}
      <div className="flex justify-between items-start border-b border-[#9CA3AF]/18 pb-4">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-[#00E676]">
        ...
        <div className="flex items-center gap-3">
          <span className="font-mono text-[10px] font-medium px-2 py-0.5 border border-[#00E676]/30 bg-[#00E676]/10 text-[#00E676] rounded-sm uppercase tracking-wider">
            ● {data.status}
          </span>
          <button
            onClick={() => {
            ...
            className="font-mono text-[11px] text-[#9CA3AF] hover:text-[#F5F5F3] border border-[#9CA3AF]/18 hover:border-[#00E676]/40 px-2 py-0.5 rounded-sm transition-colors cursor-pointer"
            title="Close"
          >
            [ X ]
          </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {data.metrics.map((m, idx) => (
            ...
            >
              ...
              <span className="text-[#00E676] font-medium text-[11px] mt-1">{m.value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {data.details && (
        <div className="bg-[#121418] border border-[#9CA3AF]/18 p-3 rounded-sm font-mono text-[10px] space-y-1">
          <div className="text-[#00E676] text-[9px] uppercase tracking-wider mb-1">
      ...
      <div className="flex justify-between items-center pt-2 border-t border-[#9CA3AF]/18 font-mono text-[10px]">
        ...
        <button
          ...
          className="text-[#00E676] hover:text-[#F5F5F3] font-medium transition-colors flex items-center gap-1.5 cursor-pointer uppercase tracking-wider"
        >
          &lt;- Return to Core View
        </button>
      </div>
