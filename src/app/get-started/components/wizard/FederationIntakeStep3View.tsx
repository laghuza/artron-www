"use client";

import Link from "next/link";
import DiagnosticCell from "../ui/DiagnosticCell";
import ExecutiveEncryptionKeyInput from "../ui/ExecutiveEncryptionKeyInput";
import { formatPhone } from "./wizardUtils";

interface FederationIntakeStep3ViewProps {
  firstName: string; setFirstName: (v: string) => void;
  lastName: string; setLastName: (v: string) => void;
  execPosition: string; setExecutivePosition: (v: string) => void;
  contactMobile: string; setContactMobile: (v: string) => void;
  officialEmail: string; setOfficialEmail: (v: string) => void;
  accessCode: string; setAccessCode: (v: string) => void;
  showPassword: boolean; setShowPassword: (v: boolean) => void;
  isAgreed: boolean; setIsAgreed: (v: boolean) => void;
  personalId: string; setPersonalId: (v: string) => void;
  isBiometricAgreed: boolean; setIsBiometricAgreed: (v: boolean) => void;
  isFedStep3Valid: boolean;
  setStep: (s: number) => void;
  onSubmit: () => void;
  isSubmitting?: boolean;
  submitError?: string | null;
}

export default function FederationIntakeStep3View({
  firstName, setFirstName, lastName, setLastName, execPosition, setExecutivePosition,
  contactMobile, setContactMobile, officialEmail, setOfficialEmail, accessCode, setAccessCode,
  showPassword, setShowPassword, isAgreed, setIsAgreed, personalId, setPersonalId,
  isBiometricAgreed, setIsBiometricAgreed, isFedStep3Valid, setStep, onSubmit,
  isSubmitting = false, submitError = null
}: FederationIntakeStep3ViewProps) {
  return (
    <div className="space-y-4 flex-1 flex flex-col justify-between">
      <div>
        <div className="space-y-1 mb-4">
          <div className="font-mono text-[9.5px] text-[#9CA3AF] uppercase tracking-widest flex justify-between">
            <span>[ CALIBRATION_SEQUENCE: NODE_03_OF_03 ]</span>
            <span><span className="text-[#00ff87]">● ● ●</span></span>
          </div>
          <h2 className="text-2xl font-semibold tracking-tight text-[#F5F5F7] uppercase">EXECUTIVE AUTHORITY</h2>
        </div>
        <div className="space-y-1">
          <DiagnosticCell coordinate="[ REP_IDENTITY // FIRST_NAME ]" placeholder="ნიკოლოზ" value={firstName} onChange={setFirstName} isValid={firstName.trim() !== ""} />
          <DiagnosticCell coordinate="[ REP_IDENTITY // LAST_NAME ]" placeholder="ყიფიანი" value={lastName} onChange={setLastName} isValid={lastName.trim() !== ""} />
          <DiagnosticCell coordinate="[ DEPLOYED_ROLE // OFFICIAL_POSITION ]" placeholder="პრეზიდენტი / გენერალური მდივანი" value={execPosition} onChange={setExecutivePosition} isValid={execPosition.trim() !== ""} />
          <DiagnosticCell coordinate="[ COMM_LINK // MOBILE_PHONE ]" placeholder="+995 (599) 12 34 56" value={contactMobile} onChange={(val) => setContactMobile(formatPhone(val))} isValid={contactMobile.replace(/\D/g, '').length === 12} />
          <DiagnosticCell coordinate="[ ACCESS_GATEWAY // OFFICIAL_EMAIL ]" placeholder="OFFICE@FEDERATION.GE" value={officialEmail} onChange={setOfficialEmail} isValid={officialEmail.includes("@")} />
          <DiagnosticCell coordinate="[ SECURITY // REPRESENTATIVE_PERSONAL_ID ]" placeholder="პირადი ნომერი (11 ციფრი)" value={personalId} onChange={setPersonalId} isValid={/^\d{11}$/.test(personalId)} />
          <ExecutiveEncryptionKeyInput accessCode={accessCode} setAccessCode={setAccessCode} showPassword={showPassword} setShowPassword={setShowPassword} />
          <div className="border border-dashed border-[#9CA3AF]/10 bg-[#121418]/30 p-3 mb-4 rounded-none space-y-3">
            <span className="text-[#9CA3AF] text-[8.5px] font-mono block tracking-widest uppercase mb-1">[ PROTOCOL_09 // COMPLIANCE_AND_CONSENT_REGISTRY ]</span>
            <div className="flex items-start gap-2">
              <button type="button" onClick={() => setIsAgreed(!isAgreed)} className="text-[#00ff87] hover:text-white font-mono text-xs focus:outline-none cursor-pointer mt-0.5">{isAgreed ? '[ X ]' : '[   ]'}</button>
              <p className="text-[10px] text-[#9CA3AF] uppercase leading-relaxed tracking-wider">მე ვადასტურებ, რომ მაქვს უფლებამოსილება წარმოვადგინო ეს ფედერაცია და ვეთანხმები Artron-ის <Link href="/sla" target="_blank" className="text-[#00ff87] hover:underline transition-all cursor-pointer font-semibold">[SLA]</Link> და <Link href="/privacy" target="_blank" className="text-[#00ff87] hover:underline transition-all cursor-pointer font-semibold">[კონფიდენციალურობის პოლიტიკას]</Link>.</p>
            </div>
            <div className="flex items-start gap-2 pt-2 border-t border-[#9CA3AF]/10">
              <button type="button" onClick={() => setIsBiometricAgreed(!isBiometricAgreed)} className="text-[#00ff87] hover:text-white font-mono text-xs focus:outline-none cursor-pointer mt-0.5">{isBiometricAgreed ? '[ X ]' : '[   ]'}</button>
              <p className="text-[10px] text-[#9CA3AF] uppercase leading-relaxed tracking-wider">მე ვაცხადებ თანხმობას ბიომეტრიული მონაცემების (სახის იდენტიფიკაცია ტურნიკეტზე) დამუშავებაზე დაცული ფიზიკური წვდომის კონტროლისთვის.</p>
            </div>
          </div>
        </div>
      </div>
      {submitError && (
        <div className="p-3 border border-red-500/20 bg-red-500/5 text-red-400 font-mono text-[11px] uppercase tracking-wider leading-relaxed animate-fadeIn">
          [ ERROR ]: {submitError}
        </div>
      )}

      <div className="space-y-3">
        <div className="p-3 border border-[#9CA3AF]/10 bg-[#121418]/50 text-[#9CA3AF] font-mono text-[11px] uppercase tracking-wider leading-relaxed">[ SYSTEM_LOG ]: იქმნება ფედერაციის მმართველი ადმინისტრატორის ანგარიში. ამ მომხმარებელს მიენიჭება სრული კრიპტოგრაფიული გასაღები ფედერაციის ადმინ-პანელზე.</div>
        <div className="flex gap-4 pt-2 font-mono text-xs">
          <button type="button" disabled={isSubmitting} onClick={() => setStep(2)} className="flex-1 py-3 px-4 border border-[#9CA3AF]/20 text-[#9CA3AF] hover:bg-[#9CA3AF]/10 transition-all uppercase cursor-pointer">[ BACK ]</button>
          <button type="button" disabled={!isFedStep3Valid || isSubmitting} onClick={onSubmit} className={`flex-1 py-3 px-4 font-bold border transition-all uppercase cursor-pointer ${isFedStep3Valid && !isSubmitting ? 'border-[#00ff87] text-[#00ff87] bg-[#00ff87]/10 hover:bg-[#00ff87] hover:text-[#121418]' : 'border-[#9CA3AF]/10 text-[#9CA3AF]/30 cursor-not-allowed'}`}>
            {isSubmitting ? '[ REGISTERING... ]' : '[ REGISTER_CORE ]'}
          </button>
        </div>
      </div>
    </div>
  );
}
