...
import ExecutiveEncryptionKeyInput from "../ui/ExecutiveEncryptionKeyInput";
...
interface ClubIntakeStep3ViewProps {
  ...
  clubPersonalId: string; setClubPersonalId: (v: string) => void;
  clubIsBiometricAgreed: boolean; setClubIsBiometricAgreed: (v: boolean) => void;
...
export default function ClubIntakeStep3View({
  ...
  clubPersonalId, setClubPersonalId, clubIsBiometricAgreed, setClubIsBiometricAgreed,
  isClubStep3Valid, setStep, onSubmit, isSubmitting = false, submitError = null
    ...
    <div className="space-y-4 flex-1 flex flex-col justify-between">
      <div>
        ...
        <div className="space-y-1">
          ...
          <DiagnosticCell coordinate="[ SECURITY // REPRESENTATIVE_PERSONAL_ID ]" placeholder="პირადი ნომერი (11 ციფრი)" value={clubPersonalId} onChange={setClubPersonalId} isValid={/^\d{11}$/.test(clubPersonalId)} />
          ...
          <div className="border border-dashed border-[#9CA3AF]/10 bg-[#121418]/30 p-3 mb-4 rounded-none space-y-3">
            <span className="text-[#9CA3AF] text-[8.5px] font-mono block tracking-widest uppercase mb-1">[ PROTOCOL_09 // COMPLIANCE_AND_CONSENT_REGISTRY ]</span>
            <div className="flex items-start gap-2">
              <button type="button" onClick={() => setClubIsAgreed(!clubIsAgreed)} className="text-[#00ff87] hover:text-white font-mono text-xs focus:outline-none cursor-pointer mt-0.5">{clubIsAgreed ? '[ X ]' : '[   ]'}</button>
              <p className="text-[10px] text-[#9CA3AF] uppercase leading-relaxed tracking-wider">მე ვადასტურებ, რომ მაქვს უფლებამოსილება წარმოვადგინო ეს კლუბი და ვეთანხმები Artron-ის <Link href="/sla" target="_blank" className="text-[#00ff87] hover:underline transition-all cursor-pointer font-semibold">[SLA]</Link> და <Link href="/privacy" target="_blank" className="text-[#00ff87] hover:underline transition-all cursor-pointer font-semibold">[კონფიდენციალურობის პოლიტიკას]</Link>.</p>
            </div>
            <div className="flex items-start gap-2 pt-2 border-t border-[#9CA3AF]/10">
              <button type="button" onClick={() => setClubIsBiometricAgreed(!clubIsBiometricAgreed)} className="text-[#00ff87] hover:text-white font-mono text-xs focus:outline-none cursor-pointer mt-0.5">{clubIsBiometricAgreed ? '[ X ]' : '[   ]'}</button>
              <p className="text-[10px] text-[#9CA3AF] uppercase leading-relaxed tracking-wider">მე ვაცხადებ თანხმობას ბიომეტრიული მონაცემების (სახის იდენტიფიკაცია ტურნიკეტზე) დამუშავებაზე დაცული ფიზიკური წვდომის კონტროლისთვის.</p>
            </div>
          </div>
        </div>
      </div>
      {submitError && (
        <div className="p-3 border border-red-500/20 bg-red-500/5 text-red-400 font-mono text-[11px] uppercase tracking-wider leading-relaxed animate-fadeIn">
          [ ERROR ]: {submitError}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-4 pt-2 font-mono text-xs">
          <button type="button" disabled={isSubmitting} onClick={() => setStep(2)} className="flex-1 py-3 px-4 border border-[#9CA3AF]/20 text-[#9CA3AF] hover:bg-[#9CA3AF]/10 transition-all uppercase cursor-pointer">[ BACK ]</button>
          <button type="button" disabled={!isClubStep3Valid || isSubmitting} onClick={onSubmit} className={`flex-1 py-3 px-4 font-bold border transition-all uppercase cursor-pointer ${isClubStep3Valid && !isSubmitting ? 'border-[#00ff87] text-[#00ff87] bg-[#00ff87]/10 hover:bg-[#00ff87] hover:text-[#121418]' : 'border-[#9CA3AF]/10 text-[#9CA3AF]/30 cursor-not-allowed'}`}>
            {isSubmitting ? '[ REGISTERING... ]' : '[ REGISTER_CORE ]'}
          </button>
        </div>
      </div>
    </div>
