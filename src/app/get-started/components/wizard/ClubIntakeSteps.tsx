...
export default function ClubIntakeSteps(props: ClubIntakeStepsProps) {
  ...
  const isClubStep3Valid = props.clubFirstName.trim() !== "" && 
                           props.clubLastName.trim() !== "" && 
                           props.clubExecPosition.trim() !== "" && 
                           props.clubContactMobile.replace(/\D/g, '').length === 12 && 
                           props.clubOfficialEmail.includes("@") && 
                           props.clubAccessCode.trim() !== "" && 
                           props.clubIsAgreed && 
                           /^\d{11}$/.test(props.clubPersonalId) && 
                           props.clubIsBiometricAgreed;
    ...
    <div className="flex-1 flex flex-col justify-between max-h-[68vh] overflow-y-auto pr-1 scrollbar-none gap-3">
      ...
      {props.step === 3 && (
        <ClubIntakeStep3View
          ...
          clubPersonalId={props.clubPersonalId} setClubPersonalId={props.setClubPersonalId}
          clubIsBiometricAgreed={props.clubIsBiometricAgreed} setClubIsBiometricAgreed={props.setClubIsBiometricAgreed}
          isClubStep3Valid={isClubStep3Valid} setStep={props.setStep}
