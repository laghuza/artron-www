...
export default function FederationIntakeSteps(props: FederationIntakeStepsProps) {
  ...
  const isFedStep3Valid = props.firstName.trim() !== "" && 
                          props.lastName.trim() !== "" && 
                          props.execPosition.trim() !== "" && 
                          props.contactMobile.replace(/\D/g, '').length === 12 && 
                          props.officialEmail.includes("@") && 
                          props.accessCode.trim() !== "" && 
                          props.isAgreed && 
                          /^\d{11}$/.test(props.personalId) && 
                          props.isBiometricAgreed;
    ...
    <div className="flex-1 flex flex-col justify-between max-h-[68vh] overflow-y-auto pr-1 scrollbar-none gap-3">
      ...
      {props.step === 3 && (
        <FederationIntakeStep3View
          ...
          personalId={props.personalId} setPersonalId={props.setPersonalId}
          isBiometricAgreed={props.isBiometricAgreed} setIsBiometricAgreed={props.setIsBiometricAgreed}
          isFedStep3Valid={isFedStep3Valid} setStep={props.setStep}
