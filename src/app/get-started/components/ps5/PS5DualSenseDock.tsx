...
export const PS5DualSenseDock: React.FC<PS5DualSenseDockProps> = ({
  ...
  const { t } = useLanguage();
  const labelToUse = nextLabel || t('ps5_onboarding.dock_btn_next');
    ...
    <div className="w-full py-3 px-4 sm:px-8 border-t border-white/[0.08] bg-[#050811]/90 backdrop-blur-xl flex flex-wrap items-center justify-between gap-4 z-20 sticky bottom-0">
      ...
      <div className="hidden lg:flex items-center gap-6 text-[11px] font-mono text-slate-400">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-300">{t('ps5_onboarding.dock_enter')}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-300">{t('ps5_onboarding.dock_esc')}</span>
        </div>
        ...
        {onPlanToggle && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-300">{t('ps5_onboarding.dock_space')}</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-400">{t('ps5_onboarding.dock_os_version')}</span>
        </div>
      </div>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span>{t('ps5_onboarding.dock_submitting')}</span>
              </>
            ) : (
              <>
