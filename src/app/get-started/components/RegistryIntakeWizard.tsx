...
import { registerFederationAction, registerClubAction } from '../actions';
...
export default function RegistryIntakeWizard({ onReset }: RegistryIntakeWizardProps) {
  ...
  const [clubPersonalId, setClubPersonalId] = useState("");
  const [clubIsBiometricAgreed, setClubIsBiometricAgreed] = useState(false);


  const handleFederationSubmit = async () => {
    setIsSubmitting(true);
    setSubmitError(null);
    try {
      const res = await registerFederationAction({
        fedName,
        legalForm,
        fedCode,
        sportsType,
      ...
      setSubmitError(err.message || "სისტემური შეცდომა.");
    } finally {
    ...
    <div className="space-y-4 font-sans select-none animate-fadeIn max-w-md w-full min-h-[460px] flex flex-col justify-between">
        ...
        <div className="flex-1 flex flex-col justify-between h-full">
          <div>
            ...
            <div className="flex flex-col gap-4">
              <div onClick={() => { setFlow('federation'); setStep(1); }} className="group border border-[#9CA3AF]/10 bg-[#121418]/40 hover:border-[#00ff87] hover:bg-[#00ff87]/5 p-4 rounded-none cursor-pointer transition-all duration-300">
                <div className="font-mono text-[10px] text-[#00ff87] uppercase tracking-widest mb-1">PROTOCOL_01 // SOVEREIGN_FEDERATION</div>
                <div className="text-[13px] font-bold text-white uppercase group-hover:text-[#00ff87]">INITIATE FEDERATION REGISTRATION</div>
              ...
              <div onClick={() => { setFlow('club'); setStep(1); }} className="group border border-[#9CA3AF]/10 bg-[#121418]/40 hover:border-[#00ff87] hover:bg-[#00ff87]/5 p-4 rounded-none cursor-pointer transition-all duration-300">
                <div className="font-mono text-[10px] text-[#00ff87] uppercase tracking-widest mb-1">PROTOCOL_02 // CLUBS_AND_ACADEMIES</div>
                <div className="text-[13px] font-bold text-white uppercase group-hover:text-[#00ff87]">INITIATE CLUB REGISTRATION</div>
          ...
          <button onClick={onReset} className="w-fit text-left text-[#9CA3AF]/50 hover:text-[#00ff87] font-mono text-[10px] uppercase transition-colors pt-6 block cursor-pointer">[ ← CANCEL_PROTOCOL // RETURN ]</button>
        </div>
      )}

      {isSuccess && (
        <div className="flex-1 flex flex-col justify-between h-full">
          <div className="space-y-4 font-mono text-xs text-[#00ff87] leading-relaxed border-t border-[#00ff87]/20 pt-4 uppercase tracking-wider">
            ...
            <div className="pt-4 normal-case text-[#9CA3AF] font-sans text-[13px] leading-relaxed">ავტომატური სისტემა ახორციელებს თქვენი იურიდიული მონაცემების ვერიფიკაციას საჯარო რეესტრში.</div>
            <div className="normal-case text-[#9CA3AF] font-sans text-[13px] leading-relaxed">ვერიფიკაციის პროტოკოლი გამოგზავნილია მითითებულ ელ-ფოსტაზე:</div>
            <div className="text-white font-mono">{flow === 'federation' ? officialEmail.toUpperCase() : clubOfficialEmail.toUpperCase()}</div>
            <div className="pt-2 font-sans text-[13px] text-[#9CA3AF]">გთხოვთ, შეამოწმოთ Inbox და დაასრულოთ ვერიფიკაცია.<span className="animate-pulse inline-block w-1.5 h-3.5 bg-[#00ff87] ml-1" /></div>
          </div>
          <div className="flex flex-col gap-2 mt-4 font-mono text-[10px]">
            <div className="border-t border-[#00ff87]/20 my-2" />
            <button onClick={onReset} className="text-[#00ff87] hover:text-[#F5F5F7] transition-all font-mono text-[10px] uppercase hover:tracking-widest cursor-pointer">&gt; [ TERMINAL_ESCAPE // RETURN_TO_CORE_DASHBOARD ]</button>
          </div>
        </div>
      )}
      ...
      {flow === 'federation' && !isSuccess && (
        <FederationIntakeSteps
          ...
          onSubmit={handleFederationSubmit}
      ...
      {flow === 'club' && !isSuccess && (
        <ClubIntakeSteps
          ...
          clubPersonalId={clubPersonalId} setClubPersonalId={setClubPersonalId}
          clubIsBiometricAgreed={clubIsBiometricAgreed} setClubIsBiometricAgreed={setClubIsBiometricAgreed}
