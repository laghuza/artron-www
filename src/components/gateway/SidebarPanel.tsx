  ...
  return (
    <aside className="w-full lg:w-[40%] h-full bg-[#1A1D23]/55 border-r border-[rgba(156,163,175,0.12)] backdrop-blur-[24px] p-6 lg:p-7 flex flex-col justify-between select-none overflow-y-auto">
      ...
      <div className="w-full flex flex-col flex-1 min-h-0 mb-6">
        ...
        <div className="w-full border-b border-[rgba(156,163,175,0.12)] pb-3.5 mb-6 flex items-center justify-between">
          ...
          <Link
            ...
            className="font-mono text-[10px] uppercase tracking-[0.18em] text-[#9CA3AF]/80 hover:text-[#00FF66] border border-[rgba(156,163,175,0.2)] hover:border-[#00FF66]/60 bg-[#121418]/80 px-3.5 py-1.8 rounded transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
          >
              ...
              {!isChoiceActive ? (
                ...
                <div className="space-y-2.5 w-full animate-fadeIn">
                  <div className="text-[10px] font-mono text-[#00FF66] tracking-[2px] uppercase mb-1 flex items-center justify-between">
                    <span>{t('actions.select_protocol')}</span>
                    <button
                      onClick={() => {
                        soundEngine.playPulseNode();
                        setIsChoiceActive(false);
                  ...
                  >
                    <span>{t('actions.registered_b2b')}</span>
          ...
          <div className="animate-fadeIn w-full flex-1 flex flex-col justify-between p-6 lg:p-8 bg-[#12161A]/60 border border-white/10 rounded-xl backdrop-blur-md space-y-5">
            <div className="space-y-4">
              {activeNode.id === 9 ? (
                ...
                <>
                  <div className="font-mono text-[12px] text-[#00FF66] uppercase tracking-[0.15em]">{t(`nodes.node_${activeNode.id}.tag`)}</div>
              ...
              {activeNode.id !== 9 && (
                <>
                  <p className="text-[14px] text-gray-300 leading-[1.6] font-sans">{t(`nodes.node_${activeNode.id}.description`)}</p>
                  {activeNode.subChapters && activeNode.subChapters.length > 0 && (
                    <div className="border-t border-[#262a33] pt-3">
                      <h3 className="text-[11px] font-mono uppercase tracking-wider text-gray-400 mb-2">{t('system.available_subchapters')}</h3>
                      <div className="space-y-1.5">
                        {activeNode.subChapters.map((sub) => (
                          <button key={sub.id} onClick={() => { soundEngine.playPulseNode(); onSelectSubChapter(sub.id); }} className={`w-full text-left p-2.5 rounded text-xs transition-all border ${activeSubChapterId === sub.id ? 'bg-[#16191E] border-[#00FF66] text-[#00FF66]' : 'bg-[#121418] border-[#262a33] text-gray-300 hover:text-[#00FF66]'}`}>{t(`subchapters.${sub.id}`) || sub.title}</button>
                        ))}
                      </div>
                    </div>
                  )}
