  ...
  return (
    <aside className="w-full h-full bg-[#1A1D23]/65 border-r border-[rgba(156,163,175,0.12)] backdrop-blur-[24px] p-5 lg:p-6 flex flex-col justify-between select-none overflow-y-auto">
      ...
      <div className="w-full flex flex-col flex-1 min-h-0 mb-4">
        {/* Unified Sleek Header Controls */}
        <div className="w-full border-b border-white/10 pb-3 mb-4 flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={toggleMute}
              className="inline-flex items-center justify-center font-mono text-[10px] uppercase tracking-[0.15em] text-[#94A3B8] hover:text-white border border-white/10 hover:border-[#00A3FF]/60 bg-[#0c1017]/80 hover:bg-[#0c1017] px-2.5 py-1.5 rounded-lg transition-all cursor-pointer shadow-sm"
            >
          ...
          <Link
            ...
            className="font-mono text-[10px] uppercase tracking-[0.15em] text-gray-400 hover:text-[#00A3FF] border border-white/10 hover:border-[#00A3FF]/40 bg-[#0c1017]/80 px-2.5 py-1.5 rounded-lg transition-all flex items-center gap-1 cursor-pointer shadow-sm"
          >
            <span>🏠 {t('hud.return_to_main') || 'მთავარი'}</span>
          </Link>
        </div>
