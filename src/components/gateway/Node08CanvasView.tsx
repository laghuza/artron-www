...
import { NODE_08_SUB_NODES_DATA } from '@/data/node08SecurityData';
import { NodeCardDetailStudio } from './NodeCardDetailStudio';
import { soundEngine } from '@/core';

interface Node08CanvasViewProps {
  activeSubChapterId: string | null;
}

export const Node08CanvasView: React.FC<Node08CanvasViewProps> = ({ activeSubChapterId }) => {
  const { t } = useI18n();
  const [selectedCardIndex, setSelectedCardIndex] = useState<1 | 2 | 3 | null>(null);

  const isSubNodeSelected = Boolean(activeSubChapterId && NODE_08_SUB_NODES_DATA[activeSubChapterId]);
  const activeSubData = activeSubChapterId ? NODE_08_SUB_NODES_DATA[activeSubChapterId] : null;
    ...
    <div className="absolute inset-0 pointer-events-none z-30 flex flex-col justify-between p-4 lg:p-7 animate-fadeIn">
        ...
        <div className="w-full flex items-center justify-between pointer-events-auto">
          <div
            className={`transition-all duration-500 ${
              isSubNodeSelected
                ? 'transform translate-y-0 opacity-100 bg-[#0E1015]/90 border border-emerald-500/30 px-4 py-2 rounded-lg shadow-[0_0_15px_rgba(0,255,135,0.15)] backdrop-blur-md'
      ...
      {selectedCardIndex !== null ? (
        ...
        <div className="w-full h-full pointer-events-auto my-auto animate-fadeIn">
          <NodeCardDetailStudio
            nodeId={8}
            nodeTitle={t('node_08_security.title')}
        ...
        <div className="my-auto mx-auto max-w-2xl text-center pointer-events-auto bg-[#0A0D11]/85 border border-emerald-500/30 p-7 lg:p-9 rounded-2xl backdrop-blur-xl shadow-[0_12px_40px_rgba(0,0,0,0.6)] animate-fadeIn">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 font-mono text-[11px] text-[#00ff87] uppercase tracking-[0.2em] mb-4">
            ...
            [ NODE_08 // SECURITY, GDPR &amp; LEGAL SHIELD ]
          </div>
        ...
        <div className="my-auto w-full max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-5 pointer-events-auto animate-fadeIn">
          ...
          <div
            ...
            className="group bg-[#0E1015]/90 border border-emerald-500/20 hover:border-[#00ff87] rounded-xl p-5 lg:p-6 backdrop-blur-xl shadow-[0_8px_30px_rgba(0,0,0,0.5)] hover:shadow-[0_0_30px_rgba(0,255,135,0.3)] transition-all duration-300 flex flex-col justify-between cursor-pointer hover:scale-[1.02]"
          >
            <div>
              <div className="font-mono text-[12px] font-bold text-[#00ff87] uppercase tracking-wider mb-3 flex items-center justify-between border-b border-white/10 pb-2.5">
                <span>01 // {t('node_08_security.section_functional')}</span>
              ...
              <p className="font-sans text-[13.5px] text-gray-200 leading-[1.65]">
                {subIndex !== undefined ? t(`node_08_security.sub_items.${subIndex}.functional_desc`) : activeSubData?.card1.desc}
              </p>
            </div>
            <div className="mt-5 pt-3 border-t border-white/5 font-mono text-[10px] text-[#00ff87] uppercase tracking-widest flex items-center justify-between">
              <span>AES-256 შიფრაცია →</span>
              <span>MOD 08.A</span>
            </div>
          </div>
          ...
          >
            <div>
              <div className="font-mono text-[12px] font-bold text-[#00ff87] uppercase tracking-wider mb-3 flex items-center justify-between border-b border-white/10 pb-2.5">
                <span>02 // {t('node_08_security.section_permissions')}</span>
                <span className="w-1.5 h-1.5 rounded-full bg-[#00ff87] opacity-60 group-hover:opacity-100 transition-opacity" />
              </div>
              <p className="font-sans text-[13px] text-gray-300 leading-[1.65] font-mono border-l-2 border-[#00ff87]/40 pl-3">
                {subIndex !== undefined ? t(`node_08_security.sub_items.${subIndex}.permissions`) : activeSubData?.card2.desc}
              </p>
            </div>
            <div className="mt-5 pt-3 border-t border-white/5 font-mono text-[10px] text-[#00ff87] uppercase tracking-widest flex items-center justify-between">
              <span>RLS + PURGE ENGINE →</span>
              <span>MOD 08.B</span>
            </div>
          </div>
          ...
          >
            <div>
              <div className="font-mono text-[12px] font-bold text-[#00ff87] uppercase tracking-wider mb-3 flex items-center justify-between border-b border-white/10 pb-2.5">
                <span>03 // {t('node_08_security.section_business')}</span>
                <span className="w-1.5 h-1.5 rounded-full bg-[#00ff87] opacity-60 group-hover:opacity-100 transition-opacity" />
              </div>
              <p className="font-sans text-[13.5px] text-gray-200 leading-[1.65]">
                {subIndex !== undefined ? t(`node_08_security.sub_items.${subIndex}.business_value`) : activeSubData?.card3.desc}
              </p>
            </div>
            <div className="mt-5 pt-3 border-t border-white/5 font-mono text-[10px] text-[#00ff87] uppercase tracking-widest flex items-center justify-between">
              <span>იურიდიული 0-რისკი →</span>
              <span>MOD 08.C</span>
            </div>
          </div>
        </div>
      ...
      {!selectedCardIndex && (
        <div className="w-full flex justify-end font-mono text-[10px] text-gray-500/80 tracking-widest uppercase pointer-events-none">
          <span>ARTRON OS // NODE_08 // SECURITY &amp; COMPLIANCE</span>
        </div>
      )}
    </div>
  );
