...
const COLORS: Record<number, string> = { 1: "#00B0FF", 2: "#00ff87", 3: "#D97736", 4: "#00ff87", 5: "#D4AF37", 6: "#D4AF37", 7: "#9CA3AF", 8: "#9CA3AF", 9: "#00ff87" };
...
export const NodeCanvas: React.FC<NodeCanvasProps> = ({
  ...
  const nodeItems = COORDS.map((coord, i) => ({
    id: i + 1, ...coord, label: t(`labels.node_${i + 1}`), active: currentActive === i + 1, color: COLORS[i + 1] || "#9CA3AF"
  }));
      ...
      <svg viewBox="0 0 400 400" className="w-full max-w-[460px] aspect-square cursor-pointer overflow-visible z-10" onMouseLeave={() => setHoveredNode(null)}>
        ...
        <defs>
          <radialGradient id="core-glow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor={isNode01Active ? "#00B0FF" : "#00ff87"} stopOpacity="0.45" />
            <stop offset="100%" stopColor={isNode01Active ? "#00B0FF" : "#00ff87"} stopOpacity="0" />
          </radialGradient>
        </defs>
        ...
        {(currentActive !== 0) && nodeItems.map((node) => {
          ...
          return (
            <circle key={`packet-${node.id}`} r="2.5" fill={node.id === 1 ? "#00B0FF" : "#00ff87"} className="pointer-events-none" style={{ opacity: isBlurred ? 0.05 : 1 }}>
        ...
        <g onClick={() => handleNodeSelect(9)} onMouseEnter={() => handleNodeHover(9)} onMouseLeave={() => setHoveredNode(null)} className="cursor-pointer">
          ...
          {[38, 28].map((r) => (
            <circle key={r} cx="200" cy="200" r={r} className={`fill-none ${isNode01Active ? "stroke-[#00B0FF]" : "stroke-[#00ff87]"} stroke-[0.8] opacity-25 pointer-events-none ${activeNode === 9 ? "animate-pulse" : ""}`} />
          ))}
          <circle cx="200" cy="200" r="22" fill="none" stroke={isNode01Active ? "#00B0FF" : "#00ff87"} strokeWidth="0.8" className="opacity-20 pointer-events-none animate-ping" />
          ...
          <g className="transition-transform duration-300" style={{ transform: isCenterActive ? "scale(1.2)" : "scale(1)", transformOrigin: "200px 200px" }}>
            <circle cx="200" cy="200" r="16" fill="#121418" stroke={isNode01Active ? "#00B0FF" : "#00ff87"} strokeWidth="1.2" style={{ fillOpacity: isCenterActive ? 0.9 : 0.4 }} />
            <circle cx="200" cy="200" r="6" fill={isNode01Active ? "#00B0FF" : "#00ff87"} className={isNode01Active ? "" : "animate-svg-emerald-ambient"} />
          </g>
          <text x="200" y="178" textAnchor="middle" className={`font-mono text-[7px] tracking-wider fill-${isNode01Active ? '[#00B0FF]' : '[#00ff87]'} transition-opacity duration-300 ${isCenterActive ? "opacity-100" : "opacity-0"}`}>
            {t('labels.node_9')}
          </text>
        </g>
