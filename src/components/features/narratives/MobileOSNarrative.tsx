...
export default function MobileOSNarrative() {
  ...
  const handleSubSelect = (subId: string) => {
    if (orchestrator?.selectSubModule) orchestrator.selectSubModule(subId);
  };
  ...
  return (
    <div className="space-y-4 font-sans select-none animate-fadeIn">
      ...
      <div className="flex gap-2 font-mono text-[10px]">
        {["qr_ticket", "biometric_pass", "workout_tracker"].map((sub) => (
          <button
            key={sub}
            onClick={() => handleSubSelect(sub)}
            className={`px-2 py-1 border rounded uppercase tracking-wider transition-all cursor-pointer ${
              activeSubModuleId === sub ? "border-emerald-core bg-emerald-core/20 text-emerald-core" : "border-silver-structure/20 text-silver-structure/60"
            }`}
          >
            {sub.replace("_", " ")}
          </button>
