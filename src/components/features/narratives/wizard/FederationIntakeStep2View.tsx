    ...
    <div className="space-y-4 flex-1 flex flex-col justify-between">
      <div>
        <div className="space-y-1 mb-4">
          <div className="font-mono text-[9.5px] text-[#9CA3AF] uppercase tracking-widest flex justify-between">
            <span>[ CALIBRATION_SEQUENCE: NODE_02_OF_05 ]</span>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-4 font-mono text-xs">
          ...
          <button type="button" disabled={!isFedStep2Valid} onClick={() => setStep(3)} className={`flex-1 py-3 px-4 font-bold border transition-all uppercase cursor-pointer ${isFedStep2Valid ? 'border-[#00ff87] text-[#00ff87] bg-[#00ff87]/10 hover:bg-[#00ff87] hover:text-[#121418]' : 'border-[#9CA3AF]/10 text-[#9CA3AF]/30 cursor-not-allowed'}`}>[ CONTINUE ]</button>
        </div>
      </div>
    </div>
  );
