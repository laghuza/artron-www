...
describe('Video Asset Engineering & Preload Tests (Sub-stage 1.2)', () => {
  ...
  const mp4Path = path.join(publicVideoDir, 'dark-minimalist-4k.mp4');
  const maxSizeBytes = 3.5 * 1024 * 1024; // 3.5 MB maximum constraint for high-fidelity 1080p

  test('dark-minimalist-4k.mp4 exists in public/video directory', () => {
    expect(fs.existsSync(mp4Path)).toBe(true);
  });

  test('dark-minimalist-4k.mp4 file size is compressed under 3.5 MB target', () => {
  ...
  test('RootLayout head contains high-priority video preload tags', () => {
    ...
    expect(content).toContain('rel="preload" href="/video/dark-minimalist-4k.mp4" as="video"');
  });
});
